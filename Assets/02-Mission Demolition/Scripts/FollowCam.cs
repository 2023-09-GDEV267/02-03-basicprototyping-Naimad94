...
{ 
    ...
    void FixedUpdate()
    {
        ...
        else
        {
            ...
            if(POI.tag == "Projectile") //If poi is a projectile, chack to see if it's at rest
            {
                if(POI.GetComponent<Rigidbody>().IsSleeping()) //if it is sleeping(that is, not moving)
                {
